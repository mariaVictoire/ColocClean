...
export function ValidateForm({
  ...
  function onSubmit(e: React.FormEvent) {
    ...
    if (!allRequiredOk) {
      setError("Cochez tous les points obligatoires.");
      return;
    }
    ...
    <form onSubmit={onSubmit} className="mt-5 flex flex-col gap-4">
      ...
      <label className="block text-sm font-medium text-stone-700">
        ...
        <input
          ...
          className="mt-1.5 block w-full text-sm text-stone-600 file:mr-3 file:rounded-lg file:border-0 file:bg-teal-50 file:px-3 file:py-2 file:text-sm file:font-semibold file:text-teal-800"
        />
      </label>
      ...
      >
        {pending ? "Validation…" : "Valider le ménage"}
      </button>
    </form>
  );
}
